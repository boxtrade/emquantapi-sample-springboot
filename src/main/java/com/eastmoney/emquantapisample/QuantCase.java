...
public class QuantCase {
    ...
    public void init(String userAPIPath, String jniEnvPath) {
        ...
        if (userAPIPath == null || "".equals(userAPIPath.trim())) {
            userAPI = new UserAPI(ExplainClient.getSystemTaskQueue());
        } else {
            userAPI = new UserAPI(userAPIPath, ExplainClient.getSystemTaskQueue());
        }
        try {
            if (jniEnvPath == null || "".equals(jniEnvPath.trim())) {
                userAPI.setJniEnv("");
            } else {
                userAPI.setJniEnv(jniEnvPath);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void start(String serverlistdir,String option) {
        if (serverlistdir == null || "".equals(serverlistdir.trim())) {
    ...
    public void testCmcToDataFrame() {
        ...
        EQData cmc = userAPI.cmc(code, indicators, startDate, endDate, options, 1, 1);
        LOG.info("cmc输出结果======分隔线======");
        if (cmc != null) {
            if (cmc.getErrCode() != 0) {
                LOG.info("request Cmc Error, " + cmc.getErrString());
    ...
    public void testCsdToDataFrame() {
        ...
        EQData csd = userAPI.csd(codes, indicator, startDate, endDate, "", 1, 1);
        LOG.info("csd输出结果======分隔线======");
        if (csd != null) {
            if (csd.getErrCode() != 0) {
                LOG.info("request csd Error, " + csd.getErrString());
    ...
    //证券与指标校验函数
    public void testCfc() {
        EQCtrData cfc = userAPI.cfc("000001.SZ,000001.SH,600000.SH,000000.TEST", "CODE,NAME,TEST", "FunType=css");
        if (cfc.getErrCode() != 0) {
    ...
    //校验或补全东财证券代码函数
    public void testCec() {
        EQCtrData cec = userAPI.cec("000001.SZ,000001.SH,600000.SH,000000.TEST", "ReturnType=0");
        if (cec.getErrCode() != 0) {
